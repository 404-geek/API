...
public abstract class RESTFetch extends SourceAuthorization {
	...
	protected ResponseEntity<String> token(UrlObject object, Map<String, String> values, String message) {
		 ...
		 * 
		 * System.out.println("No internet or Timeout error"); JsonObject respBody = new
		 * JsonObject(); respBody.addProperty("code", "502");
		 * respBody.addProperty("message", "timeout or no inernet"); return
		 * ResponseEntity.status(HttpStatus.GATEWAY_TIMEOUT).body(respBody.toString());
		 * }
		 * 
		 * catch(HttpServerErrorException e) {
		 * 
		 * System.out.println("inside server error catch");
		 * System.out.println(e.getMessage()); System.out.println(e.getStatusCode());
		 * JsonObject respBody = new JsonObject(); respBody.addProperty("code", "5xx");
		 * respBody.addProperty("message", "Internal server error"); return
		 * ResponseEntity.status(HttpStatus.BAD_REQUEST).body(respBody.toString());
		 * 
		 * 
		 * }
		 * 
		 */
			...
			respBody.addProperty("message", "timeout or no inernet");
			return ResponseEntity.status(HttpStatus.GATEWAY_TIMEOUT).body(respBody.toString());
		}
	...
	protected Object bodyBuilder(String contentType, MultiValueMap<String, String> preBody, String message)
			throws Exception {
		switch (contentType) {
	...
	protected MultiValueMap<String, String> buildBody(UrlObject token, Map<String, String> credentials,
			String message) {
