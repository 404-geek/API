			...
			$("#sources > .selector").click(function () {
				...
				$.each(sources, function (key, source) {
					if (id == key) {
						...
						var url = '/validate?type=source&srcdestId=' + key;
						console.log(url);
				...
				$.each(destinations, function (key, destination) {
					...
					if (id == key) {
						...
						$("#destination-form").submit(function (e) {
							...
							var url = '/validate?type=destination&srcdestId=' + srcdestid + "&database_name=" + $('#InputDatabaseName').val() + "&db_username=" + $('#InputUsername').val() + "&db_password=" + $('#InputPassword').val() + "&server_host=" + $('#InputHostname').val() + "&server_port=" + $('#InputPort').val();
							window.open(url, "_blank", 'width=800, height=600, menubar=no, resizable=no, scrollbars=no, status=no, toolbar=no, location=no');
						});
						var dst = key;
